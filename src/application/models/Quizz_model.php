<?php
	...
	class Quizz_model extends CI_Model
	{
		...
		public function verif_doublon($id)
		{
			$this->db->select('*');
		...
		public function participer($data)
		{
			return $this->db->insert('participation', $data);
		}

		public function get_statistique($clef)
		{
			$this->db->select('note, nom, prenom');
			$this->db->from('participation');
			$this->db->join('students', 'participation.student_id = students.student_id');
			$this->db->where('clef_qcm', $clef);
			$query = $this->db->get();
			return $query->result_array();
		}

		public function get_correction($clef)
		{
			$query = $this->db->get_where('participation', array('clef_reponse'=>$clef));
			return $query->result_array();
		}
	}